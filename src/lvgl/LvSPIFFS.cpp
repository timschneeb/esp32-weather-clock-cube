...
/**
 * Register a driver for Espressif SPIFFS
 */
extern "C" void lv_fs_spiffs_init(void)
{
...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    const File file = SPIFFS.open(buf, flags);
    if(!file) {
        return nullptr;
    }
...
{
    LV_UNUSED(drv);
    const auto lf = static_cast<SdFile *>(file_p);
    lf->file.close();
    delete lf;
...
static lv_fs_res_t fs_read(lv_fs_drv_t * drv, void * file_p, void * buf, uint32_t btr, uint32_t * br)
{
    LV_UNUSED(drv);
    const auto lf = static_cast<SdFile *>(file_p);
    *br = lf->file.read(static_cast<uint8_t *>(buf), btr);
...
static lv_fs_res_t fs_seek(lv_fs_drv_t * drv, void * file_p, uint32_t pos, lv_fs_whence_t whence)
{
    ...
    const auto lf = static_cast<SdFile *>(file_p);
    const int rc = lf->file.seek(pos, mode);
...
static lv_fs_res_t fs_tell(lv_fs_drv_t * drv, void * file_p, uint32_t * pos_p)
{
    ...
    *pos_p = lf->file.position();
    return static_cast<int32_t>(*pos_p) < 0 ? LV_FS_RES_UNKNOWN : LV_FS_RES_OK;
}
