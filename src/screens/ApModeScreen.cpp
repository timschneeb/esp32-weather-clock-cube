...
ApModeScreen::ApModeScreen() {
    ...
    lv_obj_set_style_text_color(ssid_label, lv_color_hex(0xFFFFFF), LV_STATE_DEFAULT);
    lv_obj_align(ssid_label, LV_ALIGN_CENTER, 0, -30);

    lv_obj_t* pwd_label = lv_label_create(_screen);
    lv_label_set_text_fmt(pwd_label, "Pass: %s", DEFAULT_PASSWORD);
    lv_obj_set_style_text_color(pwd_label, lv_color_hex(0xFFFFFF), LV_STATE_DEFAULT);
